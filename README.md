...
2. Nach jeder Folge **Matching Night** eintragen: wer saß bei wem, wie viele Lichter gingen an. Wer keinen Platz bekommt, wird auf "sitzt allein" gesetzt. Alternativ darf ein Mann doppelt sitzen, beides sind nur Sitzordnungen und unabhängig vom Format.
3. **Match Box** eintragen, wenn ein Paar geprüft wurde. Bestätigte Paare sitzen ab dann in jeder neuen Night automatisch fest, sind nicht mehr wählbar, und ihre Lichter brennen von selbst: eingetragen wird die im TV gezeigte Gesamtzahl. Beim Bearbeiten alter Nights gilt der Wissensstand von damals.

**Reihenfolge beim Nachtragen:** erst die alten Matching Nights, dann die Match Boxen. Die App merkt sich, was wann bekannt war, und sperrt bestätigte Personen nur in Nights, die danach kommen.
...
Unter `test/browser/` liegen vier Playwright-Suiten (WebKit und Chrome): Dialoggeometrie auf drei Handygrößen, die reale Doppelmatch-Staffel, das Format ohne Match und ein voller Chrome-Durchlauf. Einrichtung einmalig mit `cd test/browser && npm i && npx playwright install webkit`. `deploy.sh` führt sie vor jedem Deploy aus. Hintergrund im Failure Log: Chrome allein verzeiht Layout-Fehler, die auf anderen Geräten den halben Dialog verschwinden lassen.

`demo.html` lädt lokal einen Beispielstand der VIP-Staffel (wird nicht mit deployt). `outputs/staffel-vip-2026-start.json` ist eine Starterdatei mit den echten Kandidaten für Daten → Laden.
...
- 2026-09-03 | Format schon wieder zu eng gedacht | Nach dem ersten Formatfehler wurde "eine geht leer aus" eingebaut und als Wahrheit der Staffel gesetzt. Die echte Staffel kombiniert aber beides: Johannes hat als Doppelmatch zwei Perfect Matches, und trotzdem sitzt in jeder Night eine Frau allein, weil es nur zehn Plätze gibt. Wahrheit und Sitzordnung sind zwei verschiedene Dinge, ich hatte sie im Modell verschweißt. | Sitzordnung vom Wahrheitsformat entkoppeln: allein oder doppelt sitzen ist immer erlaubt, das Format bestimmt nur, wie viele Matches es gibt. Der Rechenkern konnte das längst, weil er Nights nur über ihre Paare zählt. Der Fehler steckte allein in der Eingabe-Validierung.
- 2026-09-03 | Browser-Tests im Scratchpad verloren | Die Playwright-Suiten lagen im Sitzungs-Scratchpad und waren nach ein paar Tagen weggeräumt. Sie mussten aus dem Gesprächsverlauf rekonstruiert werden. | Die goldene Workspace-Regel gilt auch für Testwerkzeug: was eine Session überlebt, gehört ins Repo. Die Suiten liegen jetzt unter `test/browser/` und laufen in `deploy.sh` vor jedem Deploy.
